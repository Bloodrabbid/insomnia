...
## Основные изменения
1. **GitLab Sync (Bundled)**: Плагин синхронизации встроен в ядро. Поддерживает гранулярную загрузку (Pull) и отправку (Push) с разделением на дерево запросов и дерево переменных окружения. Позволяет выбирать режим импорта (Объединение/Перезапись) отдельно для каждой категории при загрузке.
...
10. **Quick Sidebar Actions**: Кнопки быстрого создания запроса (`+`) и дублирования (иконка копирования) прямо в дереве ресурсов. Появляются при наведении.
11. **Quick Actions (Move/Rename)**: Возможность быстро переместить запрос в любую папку коллекции или переименовать его прямо из контекстного меню вкладки или боковой панели.
...
- **Быстрые действия в сайдбаре**: `packages/insomnia/src/routes/organization.$organizationId.project.$projectId.workspace.$workspaceId.debug.tsx`
  *(Логика отображения кнопок `+` и `copy` внутри `CollectionGridListItem`)*
